...
//ENVOI DU FORMULAIRE et des FICHIERS
	if(isset($_POST) AND !empty($_POST) AND isset($_FILES) AND !empty($_FILES)){
//Vérification des données obligatoires
		if(isset($_POST['dbConf']) AND !empty($_POST['dbConf']) AND isset($_POST['mailConf']) AND !empty($_POST['mailConf']) AND isset($_POST['languageConf']) AND !empty($_POST['languageConf']) AND isset($_POST['timezoneConf']) AND !empty($_POST['timezoneConf']) AND isset($_POST['extConf']) AND !empty($_POST['extConf']) AND isset($_FILES['logo']['name']) AND !empty($_FILES['logo']['name'])){
			//IMAGE
			$targetDir = ROOT_PATH.'/kale/assets/out/img/';
			$targetFile = basename($_FILES["logo"]["name"]);
			$imageFileType = pathinfo($targetFile, PATHINFO_EXTENSION);
			$uploadOk = true;
			if(getimagesize($_FILES["logo"]["tmp_name"])[0]!=250 OR getimagesize($_FILES["logo"]["tmp_name"])[1]!=100){
				$uploadOk=false;
				$_SESSION['ALERT']['danger'][]='Image au format 250*100 requise';
			}
			if($imageFileType != "png"){
				$_SESSION['ALERT']['danger'][]='Image au format PNG requise';
				$uploadOk = false;
			}
			if($uploadOk==true){
				if(!move_uploaded_file($_FILES["logo"]["tmp_name"], $targetDir.$targetFile)){
					$_SESSION['ALERT']['danger'][]='Echec de l\'enregistrement de l\'image';
				}else{
					$file="//CONFIGURED\r\n";
					//DATABASE
					$file.="//DATABASE\r\n";
...
//WRITE FILE
					...
					fwrite($fileC, $file);
					fclose($fileC);
					$_SESSION['ALERT']['success'][]='Votre configuration à bien été enregistré, vous allez être redirigez';
					?><script type="text/javascript">setTimeout(function(){window.location='/';},3000);</script><?php
				}
			}
		}
	}
...
<html lang="fr">
	...
	<body>
		<header>
			<a href="">
				<img src="/assets/out/img/logo.png">
			</a>
		...
		<section class="main-section clearfix">
			<form method="POST" action="" class="clearfix" novalidate enctype="multipart/form-data">
				...
				<div class="grid-l-4 clearfix">
					...
					<div class="card black-all grid-l-12">
						<h3 class="center">Logo du site (FORMAT 250*100, PNG)</h3>
						<div>
							<div class="form-group">
